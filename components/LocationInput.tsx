    ...
    <div className="flex flex-col sm:flex-row items-center gap-4 relative w-full">
      <div className="relative flex-grow w-full group" ref={dropdownRef}>
        ...
        <input
          ...
          className="w-full pl-12 sm:pl-16 pr-12 py-4 sm:py-6 bg-white border-2 border-slate-100 rounded-[2rem] shadow-sm focus:ring-8 focus:ring-blue-50 focus:bg-white focus:border-blue-500 outline-none transition-all duration-300 placeholder-slate-400 text-slate-800 font-bold text-base sm:text-lg"
          disabled={isLoading}
        />
        ...
        {showSuggestions && location.length >= 2 && (
          <div className="absolute top-full left-0 right-0 mt-3 bg-white border border-slate-100 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] z-[100] overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300 backdrop-blur-sm">
            <div className="py-2 sm:py-3 max-h-[300px] sm:max-h-[400px] overflow-y-auto custom-scrollbar">
              {suggestions.length > 0 ? (
                suggestions.map((suggestion, index) => (
                  <button
                    key={index}
                    onClick={() => handleSelect(suggestion)}
                    className="w-full px-5 sm:px-8 py-3 sm:py-5 text-left hover:bg-blue-50/50 transition-all flex items-center justify-between group border-b border-slate-50 last:border-0"
                  >
                    <div className="flex items-center gap-3 sm:gap-4">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-blue-100 group-hover:text-blue-600 transition-all">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="sm:w-[18px] sm:h-[18px]"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
                      </div>
                      <span className="text-slate-700 font-bold text-sm sm:text-base group-hover:text-blue-700 transition-colors">{suggestion}</span>
                    </div>
                    <span className="text-slate-300 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="sm:w-[20px] sm:h-[20px]"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
                    </span>
                  </button>
                ))
              ) : (
                <div className="px-8 py-6 sm:py-10 text-center">
                  <p className="text-slate-400 font-medium italic text-sm">No matching locations found in our database.</p>
                  <p className="text-slate-300 text-[10px] sm:text-xs mt-2 uppercase tracking-widest font-black">Try another neighborhood</p>
                </div>
              )}
            </div>
          </div>
